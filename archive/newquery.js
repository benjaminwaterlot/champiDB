...
MongoClient.connect(mongoKey, (err, client) => {
	...
	const totalGames = games28
		.find({})
		.count()
		.then(total => console.log(`Total games saved : ${total}`))

	const aggregation = games28
		.aggregate([
			{ $limit: 1000 },
			...
			{ $replaceRoot: { newRoot: '$participants' } },
			{
				$group: {
					...
					wins: {
						$sum: {
							$cond: { if: { $eq: ['$stats.win', true] }, then: 1, else: 0 },
						},
					},
					losses: {
						$sum: {
							$cond: { if: { $eq: ['$stats.win', false] }, then: 1, else: 0 },
						},
					},
